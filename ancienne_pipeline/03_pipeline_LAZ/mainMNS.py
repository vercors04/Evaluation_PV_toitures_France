...
from ancienne_pipeline.src.extraction.LAZtoMNS import *
...
def main():
    ...
    if choix == '1':
        ...
        lazToMNS(LAZ_PATH, path_out, resol=0.5, classe=6)

    elif choix == '2':
        path_out = os.path.join(OUT_DIR, f"{base}_nettoye.tif")

        mns_brut, transf = lazToMNS(LAZ_PATH, path_out, resol=0.5, classe=6)
        mns_clean = maskVegetation(mns_brut)
        mns_final = fillGaps(mns_clean)
